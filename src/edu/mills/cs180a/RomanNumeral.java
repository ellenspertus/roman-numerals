...
public class RomanNumeral {
    ...
    // Maps from 10 -> RomanNumeral(10)
    private static final Map<Integer, RomanNumeral> romanNumeralsByValue = new HashMap<>();
    // Maps from "X" -> 10, etc.
    private static final Map<String, Integer> romanNumeralValues = new HashMap<>();
    ...
    // The caller is responsible for ensuring that the value and text are correct.
    private RomanNumeral(int value, String text) {
        this.value = value;
        this.text = text;
    }

    /**
     * Provides a {@code RomanNumeral} object that represents the specified value. The argument must
     * be in the range {@link #MIN_VALUE} to {@link #MAX_VALUE}, inclusive.
     *
     * @param value the value to be represented
     * @throws IllegalArgumentException if the argument is out of bounds
     */
    public static RomanNumeral createFromValue(int value) {
        if (romanNumeralsByValue.containsKey(value)) {
            return romanNumeralsByValue.get(value);
        } else {
            RomanNumeral rn = new RomanNumeral(value);
            romanNumeralsByValue.put(value, rn);
            return rn;
        }
    }

    private RomanNumeral(String text) {
    ...
    /**
     * Provides a {@code RomanNumeral} object for the specified text. The argument must be in the
     * range {@link #MIN_VALUE} to {@link #MAX_VALUE}, inclusive.
     *
     * @param text the Roman Numeral
     * @throws IllegalArgumentException if the argument is out of bounds
     */
    public static RomanNumeral createFromString(String text) {
        int value = convertFromStringToInt(text);
        if (romanNumeralsByValue.containsKey(value)) {
            RomanNumeral rn = romanNumeralsByValue.get(value);
            return rn;
        } else {
            RomanNumeral rn = new RomanNumeral(value, text);
            romanNumeralsByValue.put(value, rn);
            return rn;
        }
    }
    ...
    protected static int convertFromStringToInt(String s) {
        ...
        if (romanNumeralValues.containsKey(s)) {
            return romanNumeralValues.get(s);
        }
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            String currentSubstring = null; // subtractive form or individual symbol starting at
                                            // index i
            ...
            if (currentValue == null) {
                throw new IllegalArgumentException(
                        "Not a valid Roman Numeral: " + currentSubstring);
            }
            ...
            if (highestSubstringSeen == null) {
                ...
                if (currentValue > highestValueSeen) {
                    throw new IllegalArgumentException(s
                            + " is not a valid Roman Numeral because substring " + currentSubstring
                            + " has a higher value (" + currentValue + ") than earlier substring "
                            + highestSubstringSeen + " (" + highestValueSeen + ")");
                }
            }
        ...
        romanNumeralValues.put(s, total); // "IX" -> 9
        return total;
    }
